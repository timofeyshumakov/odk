...
const filters = ref({
  value: {
    ...
    category: [
      ...
      { id: "LOSE, C32:APOLOGY, C32:1, C32:2, C32:3, C32:4, C32:5, C32:6, C32:7, C32:8, C32:9", title: "Отказ" },
    ],
  },
  selected: {
    assigned: [],
...
const groupedDeals = computed(() => {
  ...
  const mergedCategories = [
    ...
    { title: "Отказ", ids: ["LOSE", "C32:APOLOGY", "C32:1", "C32:2", "C32:3", "C32:4", "C32:5", "C32:6", "C32:7", "C32:8", "C32:9"] },
  ];
...
async function exportToExcel() {
  try {
    ...
    const sheetData = [];
    const merges = [];
    let rowNum = 1; // 1-based for cell addresses
    const colorMap = {
      "Потенциал": "FFF893",
    ...
    Object.keys(groupedDeals.value).forEach((catId) => {
      ...
      merges.push({ s: { r: rowNum - 1, c: 0 }, e: { r: rowNum - 1, c: 3 } });
      const titleCell = XLSX.utils.encode_cell({ r: rowNum - 1, c: 0 });
      rowNum++;
      // Header row
      sheetData.push(categoryHeaders.value.map(header => header.title));
      const headerStartRow = rowNum - 1;
      rowNum++;
      // Data rows
      const dataStartRow = rowNum - 1;
      ...
      // Total row if not 'Отказ'
      if (group.title !== 'Отказ') {
        const formattedTotal = new Intl.NumberFormat("ru-RU", { style: "currency", currency: "RUB" }).format(group.totalSum);
        sheetData.push(['Итого:', '', formattedTotal, '']);
        merges.push({ s: { r: rowNum - 1, c: 0 }, e: { r: rowNum - 1, c: 1 } });
        rowNum++;
      }
      // Empty row separator
      sheetData.push(['', '', '', '']);
    ...
    Object.keys(groupedDeals.value).forEach((catId, index) => {
      const group = groupedDeals.value[catId];
      const groupOffset = index * (group.items.length + (group.title !== 'Отказ' ? 4 : 3)); // Approximate, but better to track per group if needed
      // Since rowNum accumulates, but we can loop over sheet cells instead
    });

    // Loop over all cells to set borders and alignments (since groups are sequential)
    ...
    // Set title styles (need to identify title rows: every first row per group)
    let currentRow = 0;
    Object.keys(groupedDeals.value).forEach((catId) => {
      const group = groupedDeals.value[catId];
      ...
      if (ws[titleCell]) {
        ws[titleCell].s = {
          fill: { fgColor: { rgb: colorMap[group.title] } },
      ...
      currentRow++; // header
      currentRow += group.items.length; // data
      if (group.title !== 'Отказ') {
        const itogoCell = XLSX.utils.encode_cell({ r: currentRow, c: 0 });
        if (ws[itogoCell]) {
          ws[itogoCell].s.alignment = { horizontal: "left" };
        }
        currentRow++; // total
      }
      currentRow++; // empty
...
const getData = async () => {
  ...
  const filterCategory = ["C32:UC_LXYCFO", "C32:UC_VJZ0FL", "C32:UC_6VDO9F", "C32:UC_5BBXZ5", "C32:UC_R5DX1H", "LOSE", "C32:APOLOGY", "C32:1", "C32:2", "C32:3", "C32:4", "C32:5", "C32:6", "C32:7", "C32:8", "C32:9"];//filters.value.selected.category.length === 0 ? filters.value.value.category.map(item => item.id) : filters.value.selected.category;
        ...
        try {
          const response = await fetch("https://master.rymar-consulting.ru/request/handler.php", {
            ...
            body: JSON.stringify({
              ...
              "select": ["UF_CRM_1744096783472", 'UF_CRM_1742797326', "STAGE_ID", "ASSIGNED_BY_ID", 'UF_CRM_1744890618774', 'UF_CRM_1744062581756', 'UF_CRM_1745995594', 'UF_CRM_1744064620850', 'UF_CRM_1744095783871', 'UF_CRM_1742906712910', "UF_CRM_1745222013992", "UF_CRM_1742971372921", "UF_CRM_1742972105926", "UF_CRM_1742972167794", "UF_CRM_1745308616558", "COMMENTS", "UF_CRM_1756807710"],
              "start": start,
              "limit": batchSize
            })
          });
  ...
  const contactIds = Array.from(new Set(dealsLocal.map(deal => deal.UF_CRM_1756807710).filter(id => id)));
  const contacts = await callApi("crm.contact.list", {ID: contactIds}, ["NAME", "LAST_NAME", "SECOND_NAME"]);

/*
  for (const contactId of contactIds) {
    const contact = await callApi("crm.contact.list", {}, []);//getContactById(contactId);
    if (contact) {
...
console.log(obj);
    ...
    obj.ASSIGNED_BY_ID = displayFullName(user.LAST_NAME ? user.LAST_NAME : "", user.NAME ? user.NAME : "", user.SECOND_NAME ? user.SECOND_NAME : "");
    obj.status = status && status.title ? status.title : "";
    obj.stage = stageMap(obj.STAGE_ID);
    const contact = contacts.find(con => con.ID == obj.UF_CRM_1756807710);
    if (contact) {
      obj.keyPerson = displayFullName(contact.LAST_NAME ? contact.LAST_NAME : "", contact.NAME ? contact.NAME : "", contact.SECOND_NAME ? contact.SECOND_NAME : "");
    } else {
